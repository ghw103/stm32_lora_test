 ...
 *
 * \param [OUT] Time in Ticks to wait for the next transmission according to the duty
 *              cycle.
 *
...
static void OnRxWindow1TimerEvent( void )
{
    ...
    RxWindowSetup( 923300000 + ( Channel % 8 ) * 600000, datarate, bandwidth, symbTimeout, false );
#else
    #error "Please define a frequency band in the compiler options."
#endif
}
...
static void RxWindowSetup( uint32_t freq, int8_t datarate, uint32_t bandwidth, uint16_t timeout, bool rxContinuous )
{
    uint32_t downlinkDatarate = (uint32_t) Datarates[datarate];
    RadioModems_t modem;
